...
class FeatureSelector():
    ...
    def identify_single_unique(self):
        """Finds features with only a single unique value. NaNs do not count as a unique value. 
           思路：先得到每个特征唯一值的数量，取出唯一值数量为1的特征，将其加入到筛选集合中
        """

        # Calculate the unique counts in each column
        unique_counts = self.data.nunique()  # 求出每个特征的不同取值数目（去重）
...
correlation_threshold表示特征之间的相 关性
...
#  要删除的的列['missing', 'single_unique', 'collinear', 'zero_importance', 'low_importance']
